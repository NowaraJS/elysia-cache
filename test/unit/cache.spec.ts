...
describe.concurrent('Cache Module', () => {
	...
	test('should not cache routes without isCached', async () => {
		...
		// Non-cached route
		const nonCachedResponse: Response = await app.handle(
			new Request('http://localhost/not-cached')
		);
	...
	test(
		'should override global cache TTL on specific route',
		async () => {
			const store = new MemoryStore();
			const window = 2;
			const app = new Elysia()
			...
			// /override-ttl - MISS
			const response1: Response = await app.handle(
				new Request('http://localhost/override-ttl')
			);
			expect(response1.headers.get('x-cache')).toBe('MISS');
	...
	test('should handle different HTTP methods separately', async () => {
		...
		// GET request - MISS
		const getResponse1: Response = await app.handle(
		...
		// GET request - HIT
		const getResponse2: Response = await app.handle(
			new Request('http://localhost/test', { method: 'GET' })
		);
		...
		const postResponse1: Response = await app.handle(
		...
		// POST request - HIT
		const postResponse2: Response = await app.handle(
			new Request('http://localhost/test', { method: 'POST' })
		);
